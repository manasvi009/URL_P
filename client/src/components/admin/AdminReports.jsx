...
export default function AdminReports() {
  ...
  const addRule = async (list) => {
    ...
    const url = reportData.report.url;
    let pattern = '';
    try {
      const n = url.includes('://') ? url : `http://${url}`;
      pattern = new URL(n).hostname;
    } catch {
      pattern = url;
    }
    try {
      await adminApi.post('/admin/rules', { list, pattern, type: 'domain', description: `Added from report ${id}`, enabled: true });
      setToast({ message: `Added to ${list}`, type: 'success' });
    } catch (e) {
      setToast({ message: e.response?.data?.detail || 'Rule add failed', type: 'error' });
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-slate-800/60 border border-slate-700 rounded-xl p-5 space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button className="px-3 py-2 bg-slate-700 text-white rounded text-sm" onClick={() => addRule('blacklist')}>Add to Blacklist</button>
              <button className="px-3 py-2 bg-slate-700 text-white rounded text-sm" onClick={() => addRule('whitelist')}>Add to Whitelist</button>
            </div>
