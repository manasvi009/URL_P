  ...
  const columns = useMemo(() => [
    ...
    { key: 'email', title: 'Email' },
    {
      key: 'role', title: 'Role', render: (r) => (
        <select value={r.role || 'user'} onChange={(e) => patchUser(r._id, { role: e.target.value })} className="bg-slate-700 border border-slate-600 text-white rounded px-2 py-1 text-xs">
    ...
    { key: 'actions', title: 'Actions', render: (r) => <button onClick={() => deleteUser(r._id)} className="px-2 py-1 bg-red-700 text-white rounded text-xs">Delete</button> },
  ], [rows]);
